const iTech = {
    ...
    Helpers: {
        ...
        ScriptIntegration: function (fileName, beforeDelete = false) {
            ...
            const jsPath = `//${window.location.host}/app/page-scripts/${rootPath}/${fileName}.js?v=${$.now()}`;
            // remote insert
        ...
        Load: {
            AjaxSourced: async function ($targetTable, $reqUrl, $type = FormMethod.Post, $pageLength = 10, filterData = {}) {
                ...
                var config = {
                    ...
                    language: { url: "//" + appFolder.localization + "data-table/tr.json" },
                    ajax: {
        ...
        Init: function () {
            ...
            if (tabs.length) {
                ...
                if (typeof initialJsPath != typeof undefined) {
                    ...
                    $("#innerJS script").remove();
                    // https
                    const jsPath = `${window.location.protocol}//${window.location.host}/app/page-scripts/${initialJsPath}.js?v=${$.now()}`;
                    //// remote insert
